...
def set_end_node(onnx_end_node, tflite_out_info):
    ...
    out_value_info = helper.make_tensor_value_info(out_value_info_name, TensorProto.FLOAT,
                                                   utils.tflite2onnx_shape_map(tflite_out_info['shape'].tolist()))
...
def main(model_path, model_json_path, model_save_path, add_transpose_for_channel_last_first_issue=True):
    ...
    if add_transpose_for_channel_last_first_issue is True:
        input_tensor_value_info = helper.make_tensor_value_info('input', TensorProto.FLOAT,
        ...
        if output_node_info is not None:
            ...
            if add_transpose_for_channel_last_first_issue is True:
                out_value_info, transpose_node = build_button_transpose_node_for_channel_first_2_channel_last(
                    sub_op_node_list[-1], output_node_info)
            else:
